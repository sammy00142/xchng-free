...
export const adminChatRouter = createTRPCRouter({
  ...
  getChatsByUserId: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      }) as unknown as ChatWithRelations[];
    }),
  ...
  sendAdminMessage: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      try {
        ...
        if (!chatData || chatData.deleted || !isAdmin || !ctx.userId) {
  ...
  getAllChats: adminProcedure
    ...
    .query(async ({ ctx, input }) => {
      const chats = await ctx.db.query.chat.findMany({
        ...
        orderBy: [asc(chat.lastMessageTime), asc(chat.updatedAt)],
        limit: input.limit,
        with: {
          user: true,
          asset: true,
