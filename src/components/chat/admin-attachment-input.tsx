...
interface AdminFileAttachmentProps {
  chatId: string;
}

const AdminFileAttachment: React.FC<AdminFileAttachmentProps> = ({
  chatId,
}) => {
  ...
  const sendAdminMessage = api.adminChat.sendAdminMessage.useMutation({
    retry(failureCount) {
      if (failureCount > 3) {
        return false;
      }
    ...
    onSuccess: () => {
      utils.adminChat.getChat.invalidate({ chatId });
      utils.adminChat.getMessages.invalidate({ chatId });
    ...
    try {
      await Promise.all(
        files.map(async (file) => {
          ...
          const messageId = v4(); // Create a single message ID for all media
          const mediaId = v4();
...
export default React.memo(AdminFileAttachment);
