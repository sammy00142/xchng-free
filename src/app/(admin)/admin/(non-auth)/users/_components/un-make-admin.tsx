...
import { removeAdminAction as originalRemoveAdminAction } from "@/lib/utils/adminActions/getAllUsers";
...
const wrappedRemoveAdminAction = async (
  _prevState: typeof initialState,
  formData: FormData
) => {
  const userId = formData.get("userId") as string;
  const result = await originalRemoveAdminAction(
    userId,
...
const RemoveAdmin = ({ user, working }: { user: User; working: boolean }) => {
  const [isOpen, setIsOpen] = useState(false);
  const [randomId, setRandomId] = useState("");
  const [state, formAction] = useFormState(
    wrappedRemoveAdminAction,
    initialState
  );
  ...
  useEffect(() => {
    if (state.success) {
      postToast(
        state.message || "User removed from being an admin successfully"
      );
      setIsOpen(false);
      router.refresh();
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        ...
        >
          <UserCheck2 size={20} /> <h4>Remove admin</h4>
        </Button>
      </DialogTrigger>
      <DialogContent className="rounded-md">
        <DialogHeader>
          ...
          <DialogDescription className="text-left md:text-sm">
            ...
            from being an admin by typing this ID:
            <br />
